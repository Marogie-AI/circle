    ...
    <div ref={wrap} className="relative">
      ...
      {open ? (
        ...
        >
          ...
          >
            <DeleteMenuItem label={deleteLabel} />
          </form>
        </div>
      ) : null}
    </div>
...
/**
 * Separate component because useFormStatus only reports the pending state of a form
 * ABOVE it in the tree — calling it in PostActionsMenu itself would always read idle.
 * Deletes are slow enough (and irreversible enough) that a second click while the
 * first is in flight is worth preventing outright.
 */
function DeleteMenuItem({ label }: { label: string }) {
  ...
  return (
    <button
      ...
      disabled={pending}
      aria-busy={pending}
      className="flex w-full items-center gap-2 border-t border-hairline px-3 py-2.5 text-left text-sm text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
    >
      <DeleteIcon size={15} />
